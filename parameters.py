...
par = {
    ...
    'synapse_config'        : 'full', #'full', # full is half facilitating, half depressing. See line 295 for all options
    ...
    # Training specs
    'batch_size'            : 128, #1024,
    'num_iterations'        : 20, # 2000
    'iters_between_outputs' : 5,
    'batch_train_size'      : 128, # for some reason analysis errors
    ...
    'delay_time'            : 100,
    'test_time'             : 50,
    'variable_delay_max'    : 30, # also decreased by a factor of 10
    'mask_duration'         : 5,  # duration of traing mask after test onset
    'catch_trial_pct'       : 0.0,
...
def update_dependencies():
    ...
    synaptic_configurations = {'full' : [['facilitating' if i%2 == 0 else 'depressing' for i in range(par['n_hidden'])] for j in range(par['n_hidden'])]}
    ...
    '''
    # initialize synaptic values
    par['alpha_stf'] = np.ones((par['n_hidden'], par['n_hidden']), dtype=np.float32)
    par['alpha_std'] = np.ones((par['n_hidden'], par['n_hidden']), dtype=np.float32)
    par['U'] = np.ones((par['n_hidden'], par['n_hidden']), dtype=np.float32)
    par['syn_x_init'] = np.ones((par['batch_size'], par['n_hidden'], par['n_hidden']), dtype=np.float32)
    par['syn_u_init'] = 0.3 * np.ones((par['batch_size'], par['n_hidden'], par['n_hidden']), dtype=np.float32)
    par['dynamic_synapse'] = np.zeros((par['n_hidden'], par['n_hidden']), dtype=np.float32)

    for i in range(par['n_hidden']):
        for j in range(par['n_hidden']):
            if not par['synapse_config'] in synaptic_configurations.keys():
                par['dynamic_synapse'][i, j] = 0
            elif synaptic_configurations[par['synapse_config']][i][j] == 'facilitating':
                par['alpha_stf'][i,j] = par['dt']/par['tau_slow']
                par['alpha_std'][i,j] = par['dt']/par['tau_fast']
                par['U'][i,j] = 0.15
                par['syn_u_init'][:, i, j] = par['U'][i, j]
                par['dynamic_synapse'][i,j] = 1

            elif synaptic_configurations[par['synapse_config']][i][j] == 'depressing':
                par['alpha_stf'][i, j] = par['dt']/par['tau_fast']
                par['alpha_std'][i, j] = par['dt']/par['tau_slow']
                par['U'][i, j] = 0.45
                par['syn_u_init'][:, i, j] = par['U'][i, j]
                par['dynamic_synapse'][i, j] = 1
